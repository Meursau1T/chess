...
一个以中文着法为主的终端中国象棋。棋盘参考 [techkang/xiangqi](https://github.com/techkang/xiangqi)，默认用线条绘制，也可切换为以 `·` 表示交叉点的简洁棋盘。
...
可用命令包括 `新局`、`悔棋`、`提示`、`分析`、`停止`、`翻转`、`棋盘`、`执红`、`执黑`、`双人`、`难度 1` 至 `难度 20`、`emoji`、`中文棋子`、`帮助` 和 `退出`。输入 `/` 会打开斜杠命令列表，可用上下方向键选择并按 Enter 执行。输入 `难度` 可查看当前等级，20 级是不限制棋力。

输入区是默认焦点。按 Tab 切到棋盘后，可用方向键或 `hjkl` 移动光标，用空格选子，按 `V` 切换线条和简洁棋盘。也可在输入区执行 `棋盘` 或 `/棋盘` 完成切换。合法的空落点显示为亮色 `·`，可吃的棋子使用高亮背景。黑方路数使用阿拉伯数字，红方路数使用中文数字；翻转棋盘后标签会跟随双方位置。再次按 Tab 回到输入区。
